...
def test_parse_git_log_last_touch_newest_wins():
    ...
    got = _parse_git_log_last_touch(stdout)
    key = Path("src/foo.py")
    assert got[key][0] == "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    assert "2026-01-02" in got[key][1]


def test_last_commits_for_paths_on_repo():
    root = Path(__file__).resolve().parents[2]
    paths = [Path("src/internal/git_utils.py"), Path("pyproject.toml")]
    m = last_commits_for_paths(root, paths, INDEX_DB_BATCH_SIZE)
    for p in paths:
        h, d = m.get(p, ("", ""))
        assert len(h) == 40
        assert d
